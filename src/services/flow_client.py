...
# ===== 中文路径 SSL 证书修复 =====
# curl_cffi 内部的 libcurl 无法读取含中文字符的路径下的 cacert.pem
# 这里将证书复制到纯 ASCII 临时目录，并在创建 AsyncSession 时显式传入
import os as _os, shutil as _shutil, tempfile as _tempfile
_SAFE_CA_BUNDLE: str | None = None
try:
    ...
    try:
        ...
        _SAFE_CA_BUNDLE = _os.path.join(_safe_dir, 'cacert.pem')
        _shutil.copy2(_orig_cert, _SAFE_CA_BUNDLE)
        print(f"[SSL补丁] 证书已复制到安全路径: {_SAFE_CA_BUNDLE}")
except Exception:
    pass
# ===== 补丁结束 =====
...
class FlowClient:
    ...
    async def _make_request(
        ...
        try:
            _session_kwargs = {}
            if _SAFE_CA_BUNDLE:
                _session_kwargs['verify'] = _SAFE_CA_BUNDLE
            async with AsyncSession(**_session_kwargs) as session:
                if method.upper() == "GET":
