...
# ================================================================
# 🔧 全局补丁: 修复中文路径下 curl_cffi SSL 证书读取失败 (curl: 77)
# 必须在所有其他 import 之前执行！
# ================================================================
import os as _os, shutil as _shutil, tempfile as _tempfile
try:
    import certifi as _certifi
    _orig_cert = _certifi.where()
    _needs_patch = False
    try:
        _orig_cert.encode('ascii')
    except (UnicodeEncodeError, UnicodeDecodeError):
        _needs_patch = True

    if _needs_patch:
        _safe_dir = _os.path.join(_tempfile.gettempdir(), 'flow2api_certs')
        _os.makedirs(_safe_dir, exist_ok=True)
        _safe_cert = _os.path.join(_safe_dir, 'cacert.pem')
        _shutil.copy2(_orig_cert, _safe_cert)

        # Monkey-patch certifi.where() 本身，让所有库都读到安全路径
        _certifi.where = lambda: _safe_cert

        # Monkey-patch curl_cffi AsyncSession 的 __init__
        from curl_cffi.requests import AsyncSession as _OrigAsyncSession
        _orig_init = _OrigAsyncSession.__init__

        def _patched_init(self, *args, **kwargs):
            if 'verify' not in kwargs:
                kwargs['verify'] = _safe_cert
        ...
        # 同样 patch 同步 Session
        try:
            from curl_cffi.requests import Session as _OrigSession
            _orig_sync_init = _OrigSession.__init__
            ...
            _OrigSession.__init__ = _patched_sync_init
        except Exception:
            pass

        # 设置环境变量作为额外保险
        _os.environ['CURL_CA_BUNDLE'] = _safe_cert
        _os.environ['REQUESTS_CA_BUNDLE'] = _safe_cert
        _os.environ['SSL_CERT_FILE'] = _safe_cert

        print(f"✅ [SSL全局补丁] 证书已复制到: {_safe_cert}")
        print(f"✅ [SSL全局补丁] curl_cffi AsyncSession/Session 已全局修复")
except Exception as _patch_err:
    print(f"⚠️ [SSL全局补丁] 跳过: {_patch_err}")
# ================================================================
...
async def studio_page():
    """Google Banana & Veo Studio page"""
    studio_file = static_path / "studio.html"
    if studio_file.exists():
        return FileResponse(str(studio_file))
    return HTMLResponse(content="<h1>Studio Page Not Found</h1>", status_code=404)
